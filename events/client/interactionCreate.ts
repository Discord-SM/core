    ...
    run: async(client, interaction: Interaction) => {
        ...
        if(clientCommand) {
            ...
            if(clientCommand.permissions?.client?.length) {
                if(!guild.members.me.permissions.has(clientCommand.permissions.client)) return interaction.reply({ embeds: [client.managers.utils.buildEmbed({ description: client.phrases.permissions.missingClient }, null)] });
            }
