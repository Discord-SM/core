...
export default {
    name: 'server',
    description: require(`../locales/${locales.find(locale => locale.tag === config.defaultLocale).name}.json`).commands.server.info,
    cooldown: '10s',
        ...
        query({ type: serverType as Type, host: serverIP, port: Number(serverPort), maxAttempts: 1, attemptTimeout: 10000, socketTimeout: 1000 }).then((data: any) => {
            ...
            data.players.forEach((player: any) => {
                if(player.raw?.time) {
                    ...
                    const formattedTime = client.managers.utils.formatNumbers([hours, minutes, seconds]);
            ...
            const info = client.managers.utils.buildEmbed({ author: { name: client.phrases.commands.server.embed.title.replace('{server}', data.name), icon_url: serverLogo || client.user.avatarURL() || command.guild.iconURL() }, thumbnail: { url: serverLogo || client.user.avatarURL() || command.guild.iconURL() }, description: '-', fields: [
            ...
            const failFetchData = client.managers.utils.buildEmbed({ description: client.phrases.commands.server.failFetchData }, null);
